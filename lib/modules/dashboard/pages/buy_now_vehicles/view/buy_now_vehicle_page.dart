...
class _BuyNowVehiclePageState extends State<BuyNowVehiclePage> {
  ...
  void _scrollListener() {
    if (_scrollController.position.pixels >=
      ...
      _buyNowVehicleController.fetchMoreBuyNowVehicles(searchParams: searchParams);
    }
  }

  _initialApiCalls({bool showLoader = false}) async {
    _buyNowVehicleController.fetchFilterVehicleOptions(showLoader: showLoader);
    _buyNowVehicleController.fetchBuyNowVehicles(showLoader: showLoader,loadingInitial: true, searchParams: searchParams);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: BuyNowVehicleWidgets.appBar(
          onTapBack: (){
      ...
      body: Column(
        children: [
          // HEADER
          Obx(() =>  BuyNowVehicleWidgets.header(
              selectedView: _buyNowVehicleController.selectedView,
              onTapSwitchView: _buyNowVehicleController.toggleView,
              searchTextController: _buyNowVehicleController.searchTextController,
              onTapFilter: () async{
                if(_buyNowVehicleController.filterVehicleOptions != null){
                  searchParams = await Get.toNamed(AppRoutes.filterVehicles, parameters: {
                    'isFromBuyNowVehicles': 'true',
                  });
                }
              },
              onSearch: (value){
                _buyNowVehicleController.fetchBuyNowVehicles(loadingInitial: true, searchParams: searchParams);
              }
          ),),
                  ...
                  return RefreshIndicator.adaptive(
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: BuyNowVehicleWidgets.body(
                        vehicles: vehicles,
                        isLoadingPagination: _buyNowVehicleController.isLoadingPagination,
                        selectedView: _buyNowVehicleController.selectedView,
                        onTapFav: (isWatched, vehicleId){
                          context.showLoaderOverlay;
                          _globalController.vehicleIsWatch(vehicleId: vehicleId, isWatched: isWatched).then((response) {
